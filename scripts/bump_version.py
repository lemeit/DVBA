...
Artefactos (cada uno con su propia version):
  ...
  - guia       → docs/guia_dvba_campo.html (header)
  - bitacora   → docs/bitacora.html (header)

Uso:
  python scripts/bump_version.py --show
  python scripts/bump_version.py campo v9.3
  python scripts/bump_version.py escritorio v6.9
  python scripts/bump_version.py guia v1.1
  python scripts/bump_version.py bitacora v3.2
  python scripts/bump_version.py todos v9.3        (bumpea todos a la misma version - uso ocasional)
"""
import re, sys
...
# Por artefacto: lista de (archivo, etiqueta, regex con grupo 'v', plantilla)
ARTEFACTOS = {
    "campo": [
...
def show_one(art_name, targets):
    ...
    for fname, label, regex, _ in targets:
        ...
        if m and not ver.startswith("v") and re.match(r"^\d", ver):
            ver = "v" + ver
        print("  " + fname.ljust(34) + label.ljust(22) + ver)
...
def bump(art_name, new_ver):
    ...
    for art, targets in artefactos.items():
        for fname, label, regex, tpl in targets:
            ...
            print("  [OK] " + fname.ljust(34) + label.ljust(22) + old_norm + " -> " + new_ver)
            mods += 1
    if mods:
        print("\nProximos pasos:")
        print("  git add -A")
