...
> **Estado del sistema al armar el documento:** apps de escritorio en **v8.64**, familia móvil en **v9.91**, sello institucional en **v4** (con overlay semitransparente + EXIF metadata + QR con logo DVBA), base multi-zona activa (Fase 1 + 2 implementadas), refactor de sello unificado en un módulo compartido (elimina duplicación entre portal/partes/móvil).
...
14. **4 flujos coordinados de creación/edición de registros** (v8.64): (a) desde móvil con GPS auto-ubicado + auto-completado con armonizador, (b) desde el mapa con `📍 Nuevo Pin` + arrastre, (c) por progresiva con `🎯 Ubicar` (inverso RP+km → lat/lng con CHAIN+ANCHORS), (d) edición fluida arrastrando pins que recalcula todo al vuelo.
...
- Familia escritorio (portal, módulo Plan de Seguridad, módulo Reportes) → **v8.64**
- Familia móvil (PWA `app.html` — Modo Básico + Modo Avanzado + service worker) → **v9.91**
...
### 4.3 Portal escritorio (`index.html` v8.64)
...
### 4.4 Plan de Seguridad en la Circulación (`partes_diarios.html` v8.64)
...
### 4.5 Módulo Reportes (`reportes.html` v8.64)
...
_Última actualización: 28 de julio de 2026 · SIG Vial PBA en v8.64 (escritorio) / v9.91 (móvil)._
